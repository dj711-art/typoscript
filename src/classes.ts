import { it, eq, log } from "./testing.js";
...
abstract class Shape {
  ...
  static createShape(
    ...
    if (kind === "triangle" && third !== undefined) return new Triangle(second, third);
...
const triangleForTest = Shape.createShape("triangle", 10, 20) as Triangle;
it("set base to 0 with exception", () => {
  try {
    triangleForTest.base = 0; // should throw exception
  } catch (e) {
    return true;
  }
  return false;
...
it("set base to 3 with no exception", () => {
  triangleForTest.base = 3; // should not throw exception
  return triangleForTest.base == 3;
});

it("set height to 0 with exception", () => {
  try {
    triangleForTest.height = 0; // should throw exception
  } catch (e) {
    return true;
  }
  return false;
...
it("set height to 3 with no exception", () => {
  triangleForTest.height = 3; // should not throw exception
  return triangleForTest.height == 3;
}); 